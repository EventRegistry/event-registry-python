...
q.addRequestedResult(RequestEventsInfo(page = 0, count = 30, sortBy = "size", sortByAsc = True, 
                                       returnInfo = ReturnInfo(conceptLang = "deu", conceptTypes = ["person", "wiki"])))   # return event details for first 30 events
q.addRequestedResult(RequestEventsConceptAggr(conceptCount = 5, 
                                              returnInfo = ReturnInfo(conceptTypes = ["org", "loc"])))        # compute concept aggregate on the events
res = er.execQuery(q)
...
q.addRequestedResult(RequestEventsConceptTrends(conceptCount = 40, 
                                                returnInfo = ReturnInfo(conceptTypes=["person"])))
